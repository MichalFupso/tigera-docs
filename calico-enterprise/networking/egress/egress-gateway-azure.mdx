...
Azure offers two ways to make egress gateway addresses reachable. This page covers BGP, where
gateways use native VNet IP addresses advertised to an Azure Route Server. For the approach that
needs neither BGP nor a Route Server, see
[Configure egress gateways, Azure user-defined routes](egress-gateway-azure-udr.mdx).
